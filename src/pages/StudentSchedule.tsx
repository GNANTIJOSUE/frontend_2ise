...
const StudentSchedule = () => {
  ...
  // Récupérer l'année scolaire depuis les paramètres de l'URL ou utiliser l'année courante
  const urlSchoolYear = searchParams.get('school_year');
  const [schoolYear, setSchoolYear] = useState(urlSchoolYear || getCurrentSchoolYear());

  console.log(`[StudentSchedule] Étudiant ID: ${studentId}, Année scolaire: ${schoolYear}`);
  ...
  useEffect(() => {
    ...
    const fetchSchedule = async () => {
      try {
        ...
        console.log(`[StudentSchedule] Appel API emploi du temps pour l'étudiant ${studentId}, année scolaire: ${schoolYear}`);
        const response = await axios.get(`https://2ise-groupe.com/api/students/${studentId}/schedule?school_year=${schoolYear}`, {
  ...
  const handleSchoolYearChange = (newSchoolYear: string) => {
    ...
    // Mettre à jour l'URL avec la nouvelle année scolaire
    navigate(`/student/schedule/${studentId}?school_year=${newSchoolYear}`);
  };
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
            <FormControl sx={{ minWidth: 160 }} size="small">
              ...
              <Select
                ...
                onChange={e => handleSchoolYearChange(e.target.value as string)}
              >
