...
import {
  Box, Container, Typography, Paper, Button, TextField, Alert, Stack, Table, TableHead, TableRow, TableCell, TableBody, CircularProgress, Chip, Dialog, DialogTitle, DialogContent, DialogActions, Grid, Snackbar, FormControl, InputLabel, Select, MenuItem, TableContainer
} from '@mui/material';
...
const StudentPaymentPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(`https://2ise-groupe.com/api/students/me/details?school_year=${schoolYear}`, {
  ...
  const handlePay = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
                    'https://2ise-groupe.com/api/payments',
        {
      ...
      // Rafraîchir la liste des paiements
              const payRes = await axios.get('https://2ise-groupe.com/api/students/me/details', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleFusionPayment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
              const res = await axios.post('https://2ise-groupe.com/api/payments/fusion-init', {
        student_id: student.id,
        amount: amountToPay,
  ...
  const handleCheckFusionStatus = async () => {
    ...
    setFusionStatusMsg(null);
    try {
              const res = await axios.get(`https://2ise-groupe.com/api/payments/fusion-status/${fusionToken}`);
        ...
        <Paper elevation={3} sx={{ p: 4, borderRadius: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.10)' }}>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            {payments.length === 0 ? (
              ...
              <TableContainer sx={{ overflowX: 'auto', borderRadius: 2, boxShadow: 1, bgcolor: '#fff' }}>
                <Table size="small" sx={{ minWidth: 500 }}>
                  <TableHead>
                    <TableRow sx={{ bgcolor: '#f5f5f5' }}>
                      ...
                      <TableCell sx={{ fontWeight: 700, fontSize: { xs: 13, md: 15 }, px: { xs: 1, md: 2 } }}>Moyen de paiement</TableCell>
                  ...
                  <TableBody>
                    {payments.map((p, i) => (
                      <TableRow key={i} sx={{ '&:hover': { bgcolor: '#f8f9fa' } }}>
                        <TableCell sx={{ fontWeight: 600, fontSize: { xs: 13, md: 15 }, px: { xs: 1, md: 2 } }}>
                          {formatAmount(Number(p.montant_verse || p.amount))} FCFA
                        </TableCell>
