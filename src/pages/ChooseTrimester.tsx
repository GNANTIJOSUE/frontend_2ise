...
const ChooseTrimester = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const studentRes = await axios.get(`https://2ise-groupe.com/api/students/me?school_year=${schoolYear}`, {
        ...
        const gradesRes = await axios.get(`https://2ise-groupe.com/api/students/${studentRes.data.student.id}/grades?school_year=${schoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        if (studentRes.data.student && studentRes.data.student.class_id) {
          ...
          for (const t of ['1er trimestre', '2e trimestre', '3e trimestre']) {
            try {
              const res = await axios.get(`https://2ise-groupe.com/api/report-cards/published?class_id=${studentRes.data.student.class_id}&trimester=${encodeURIComponent(t)}&school_year=${schoolYear}`, {
                headers: { Authorization: `Bearer ${token}` }
              });
  ...
  const trimesters: TrimesterOption[] = [
    {
      ...
      average: (() => {
        ...
        const totalCoef = semesterGrades.reduce((sum, g) => sum + (g.coefficient || 1), 0);
        const totalMoyCoef = semesterGrades.reduce((sum, g) => sum + (g.moyenne * (g.coefficient || 1)), 0);
        return totalCoef > 0 ? totalMoyCoef / totalCoef : 0;
      })()
    },
    {
      id: '2e trimestre',
      ...
      gradeCount: grades.filter(g => g.semester === '2e trimestre' || g.semester === '2 ème trimestre').length,
      average: (() => {
        const semesterGrades = grades.filter(g => g.semester === '2e trimestre' || g.semester === '2 ème trimestre');
        if (semesterGrades.length === 0) return undefined;
        // Calculer la moyenne pondérée avec les coefficients
        const totalCoef = semesterGrades.reduce((sum, g) => sum + (g.coefficient || 1), 0);
        const totalMoyCoef = semesterGrades.reduce((sum, g) => sum + (g.moyenne * (g.coefficient || 1)), 0);
    ...
    },
    {
      ...
      gradeCount: grades.filter(g => g.semester === '3e trimestre' || g.semester === '3 ème trimestre').length,
      average: (() => {
        const semesterGrades = grades.filter(g => g.semester === '3e trimestre' || g.semester === '3 ème trimestre');
        if (semesterGrades.length === 0) return undefined;
        // Calculer la moyenne pondérée avec les coefficients
        const totalCoef = semesterGrades.reduce((sum, g) => sum + (g.coefficient || 1), 0);
        const totalMoyCoef = semesterGrades.reduce((sum, g) => sum + (g.moyenne * (g.coefficient || 1)), 0);
  ...
  const handleTrimesterSelect = (trimesterId: string) => {
    // Inclure l'année scolaire dans l'URL pour la transmettre à MyReportCard
    navigate(`/student/report-card/${trimesterId}?school_year=${schoolYear}`);
  };
