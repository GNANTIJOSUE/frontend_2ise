...
const AbsencesTab = ({ childId, schoolYear }: { childId: string | undefined, schoolYear: string }) => {
  ...
  useEffect(() => {
    const fetchAbsencesAndNotifications = async () => {
      ...
      const absReq = axios.get(`https://2ise-groupe.com/api/students/${childId}/absences?school_year=${schoolYear}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      // 2. Récupère les notifications du parent connecté filtrées par année scolaire
      const notifReq = axios.get(`https://2ise-groupe.com/api/events/my-notifications?school_year=${schoolYear}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
