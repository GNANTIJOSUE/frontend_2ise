...
const ReportCardTab = ({ childId, schoolYear }: { childId: string | undefined, schoolYear: string }) => {
  ...
  const pdfRef = useRef<BulletinPDFRef>(null);
  ...
  useEffect(() => {
    ...
    const fetchStudent = async () => {
      ...
      try {
        ...
        // Récupérer l'effectif de la classe si l'élève a une classe
        if (data.class_id) {
          try {
            const effectifResponse = await axios.get(`https://2ise-groupe.com/api/students/${childId}/class-effectif`, {
              headers: { Authorization: `Bearer ${token}` },
              params: {
                school_year: schoolYear
              }
            });
            const effectifData = effectifResponse.data;
            if (effectifData && effectifData.effectif) {
              data.class_size = effectifData.effectif;
            }
          } catch (effectifError) {
            console.error('Erreur lors de la récupération de l\'effectif:', effectifError);
            data.class_size = 'N/A';
          }
        }
    ...
    const fetchAllBulletins = async () => {
      ...
      let classId = null;
      try {
        const studentRes = await axios.get(`https://2ise-groupe.com/api/students/${childId}?school_year=${schoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      if (classId) {
        for (const t of ['1er trimestre', '2e trimestre', '3e trimestre']) {
          try {
            const res = await axios.get(`https://2ise-groupe.com/api/report-cards/published?class_id=${classId}&trimester=${encodeURIComponent(t)}&school_year=${schoolYear}`, {
              headers: { Authorization: `Bearer ${token}` }
            });
  ...
  const fetchClasseInfo = async (trimester: string) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await axios.get(`https://2ise-groupe.com/api/students/${childId}/trimester-rank?semester=${encodeURIComponent(trimester)}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <Box sx={{ 
      ...
      <Typography variant="h5" fontWeight={700} mb={3} color="primary.main" sx={{ px: 2 }}>
        Bulletin
      </Typography>
      {error && (
        <Box sx={{ my: 3, px: 2 }}>
      ...
      <Grid container spacing={{ xs: 2, sm: 3 }} sx={{ px: 2 }}>
        {trimesterCards.map((trim, idx) => {
          ...
          return (
            <Grid item xs={12} sm={6} md={4} key={trim.value}>
              <Paper
      ...
      {selected && bulletins[selected] && bulletins[selected].length > 0 && student && (
        ...
        >
          <Box sx={{ 
            ...
            <BulletinPDF
              key={`bulletin-${childId}-${schoolYear}-${selected}`} // Forcer le re-render complet
              ref={pdfRef}
              student={student}
